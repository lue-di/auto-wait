...
export function formatUsageReport(
	...
	else if (report.providerId === "github-copilot")
		formatGitHubCopilotReport(lines, report);
	else if (report.providerId === "openrouter")
		formatOpenRouterReport(lines, report);
	else if (report.providerId === "opencode-go")
		formatOpenCodeZenReport(lines, report);
...
export function formatUsageStatusline(
	...
	if (report.providerId === "openai-codex")
		return formatCodexStatusline(report, model);
	if (report.providerId === "github-copilot")
		return formatGitHubCopilotStatusline(report);
	if (report.providerId === "openrouter") {
		const limit = report.buckets.find((bucket) => bucket.id === "key-limit");
		if (limit?.remaining !== undefined)
			return `openrouter ${formatUsd(limit.remaining)} left`;
		const total = report.metrics.find((metric) => metric.id === "usage-total");
		if (typeof total?.value === "number")
			return `openrouter ${formatUsd(total.value)} used`;
	}
	if (report.providerId === "opencode-go")
		return formatOpenCodeZenStatusline(report);
	return undefined;
}

export function formatProviderStates(
	states: readonly ProviderUsageState[],
): string {
	return states
		.map((state) => {
			if (state.status === "ready")
				return formatUsageReport(state.report, state.displayState);
...
function formatCodexReport(lines: string[], report: UsageReport): void {
	...
	for (const metric of report.metrics) {
		if (metric.id === "reset-credits") {
			lines.push(
				`${"Usage limit resets:".padEnd(VALUE_COLUMN)}${metric.value} available`,
			);
...
function formatGitHubCopilotReport(lines: string[], report: UsageReport): void {
	...
	if (!quota || quota.limit === undefined || quota.remaining === undefined) {
		lines.push(
			`${`${quota?.label ?? "Copilot quota"}:`.padEnd(VALUE_COLUMN)}unlimited`,
		);
		return;
	}
	const percent = percentRemaining(quota);
	const reset = quota.resetsAt
		? ` (resets ${formatReset(quota.resetsAt)})`
		: "";
	lines.push(
	...
	if (typeof overage?.value === "number" && overage.value > 0) {
		lines.push(
			`${"Additional usage:".padEnd(VALUE_COLUMN)}${overage.value} ${quota.label}`,
		);
	}
}
...
function formatGitHubCopilotStatusline(report: UsageReport): string {
	...
	const overage = report.metrics.find((metric) => metric.id === "overage-used");
	const overageSuffix =
		typeof overage?.value === "number" && overage.value > 0
			? ` +${overage.value} over`
			: "";
...
function compactGitHubCopilotQuotaKind(
...
function formatOpenCodeZenReport(lines: string[], report: UsageReport): void {
	for (const bucket of report.buckets) {
		...
		lines.push(
			`${`${bucket.label}:`.padEnd(VALUE_COLUMN)}${used}% used${reset}`,
		);
	}
}
...
function formatOpenCodeZenStatusline(report: UsageReport): string | undefined {
	...
	for (const bucket of report.buckets) {
		...
		const compact =
			bucket.id === "rolling" ? "r" : bucket.id === "weekly" ? "w" : "m";
...
function formatCodexStatusline(
	...
	const parts = [
		group === "codex"
			? "codex"
			: `codex ${compactLimitLabel(labelBucket?.groupLabel ?? group)}`,
	];
...
export function findCodex5hBucket(
	report: UsageReport,
	model?: UsageModel,
	...
	return report.buckets.find(
		(bucket) =>
			(bucket.groupId ?? bucket.id) === group &&
			bucket.windowMinutes === 300 &&
			bucket.id.endsWith(":primary"),
	);
}
...
function compactLimitLabel(label: string): string {
	...
	const suffix = codex
		? normalized.slice(codex.index + codex[0].length).trim()
		: "";
