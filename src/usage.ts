...
import { registerCodexAutoWait } from "./codex-auto-wait.js";
import {
	FAST_USAGE_WARNING,
	registerCodexFastMode,
} from "./codex-fast-runtime.js";
import {
...
export default function usageExtension(
	...
	const credentialCandidates = createOAuthCredentialCandidateReader(
		pi,
		credentialReader,
	);
	const createRedemptionId = dependencies.createRedemptionId ?? randomUUID;
	const settingsRuntime =
		dependencies.settingsRuntime ?? createUsageSettingsRuntime();
	...
	let autoWaitRuntime: ReturnType<typeof registerCodexAutoWait>;
	...
	const publishStatus = (
		...
		autoWaitRuntime.observe(outcome.state.report, model);
		const rawValue = formatUsageStatusline(outcome.state.report, model);
		const fastValue = rawValue
			? fastRuntime.decorateStatus(model, rawValue)
			: undefined;
		const value = fastValue
			? autoWaitRuntime.decorateStatus(model, fastValue)
			: undefined;
	...
	const transitionCurrentIdentity = (
		nextIdentity: string,
		providerId: string,
	) => {
	...
	const queryAdapterState = async (
		...
		try {
			...
			if (isStaleExtensionContextError(error) || isAbortError(error))
				throw error;
		...
		if (displayState === "current") {
			transitionCurrentIdentity(
				`${adapter.id}:${auth.fingerprint}`,
				adapter.id,
			);
		}
		...
		try {
			const remainingMs = Math.max(
				1,
				DEFAULT_TIMEOUT_MS - (Date.now() - startedAt),
			);
	...
	const queryStableCurrent = async (
		...
		for (let attempt = 0; attempt < 3; attempt += 1) {
			...
			const result = await runMenuOperation(
				...
				async (signal) => {
					...
					return {
						outcome,
						stable: await outcomeStillCurrent(
							ctx,
							model,
							generation,
							outcome,
		...
		ctx.ui.notify(
			"The active model or account kept changing; reopen /usage to retry.",
			"warning",
		);
		return undefined;
	};

	const publishStableCurrent = (
		ctx: ExtensionCommandContext,
	...
	const showMenu = async (ctx: ExtensionCommandContext): Promise<void> => {
		...
		try {
			...
			>({
					...
					main: () => {
						...
						const fastLines =
							fastAvailability.kind === "available"
								? [
										`Fast mode: ${fastAvailability.enabled ? "On" : "Off"}`,
										FAST_USAGE_WARNING,
									]
						...
						return {
							...
							lines: [
								...formatProviderStates(visibleStates).split("\n"),
								...fastLines,
								...(ctx.model?.provider === "openai-codex"
									? [
											`Automatic 5h wait: ${autoWaitRuntime.enabled() ? "On" : "Off"}`,
										]
									: []),
							],
							items: [
								...
								...(ctx.model?.provider === "openai-codex"
									? [
											{
												id: "toggle-auto-wait",
												label: autoWaitRuntime.enabled()
													? "Turn automatic 5h wait off"
													: "Turn automatic 5h wait on",
												description:
													fastState.kind === "invalid"
														? "Repair pi-usage.json and reload before changing automatic wait."
														: "Wait for the returned 5h reset time before continuing a submitted prompt.",
												disabled: fastState.kind === "invalid",
												action: "toggle-auto-wait" as const,
											},
										]
									: []),
								...(current.state.status === "ready" &&
								current.state.providerId === "openai-codex"
									? [
											{
												id: "open-resets",
												label: REDEEM_CODEX_RESET,
												description: codexResetActionDescription(
				...
				actions: {
					"toggle-auto-wait": async () => {
						const changed = await autoWaitRuntime.toggle(
							ctx,
							!autoWaitRuntime.enabled(),
						);
						if (!changed) return { kind: "rejected" };
						fastState = settingsRuntime.get();
						if (stableCurrent) publishStableCurrent(ctx, stableCurrent);
						return { kind: "stay" };
					},
					"toggle-fast": async () => {
						...
						const changed = await fastRuntime.toggle(
							ctx,
					...
					"open-resets": async () => {
						...
						try {
							...
							return {
								kind: "to",
								screen:
									loaded.availability.availableCount > 0
										? "reset-picker"
										: "reset-result",
							};
						} catch (error) {
							...
							ctx.ui.notify(
								`Couldn't load usage limit resets: ${errorMessage(error)}`,
								"error",
							);
							return { kind: "stay" };
						}
						...
						try {
							...
							const result = await runMenuOperation(
								...
								async (signal) => {
									...
									const model = ctx.model;
									if (modelIdentity(model) !== expectedModel)
										return { outcome };
									const refreshed = await queryCurrentState(
										ctx,
										model,
										true,
										signal,
					...
					another: async () => {
						...
						if (others.length === 0) {
							ctx.ui.notify(
								"No other supported provider has configured runtime auth.",
								"info",
							);
							return { kind: "stay" };
						}
					...
					provider: async ({ itemId }) => {
						const adapter = configuredAdapters(ctx).find(
							(candidate) =>
								candidate.id === itemId && candidate.id !== ctx.model?.provider,
						);
						...
						const outcome = await runMenuOperation(
							...
							controller.signal,
							(signal) =>
								queryAdapterState(ctx, adapter, "configured", false, signal),
						);
					...
					all: async () => {
						...
						const queriedStates: ProviderUsageState[] = settled.map(
							(result, index) => {
						...
						visibleStates = [
							current.state,
							...queriedStates.filter(
								(state) => state.providerId !== current.state.providerId,
							),
						];
						return { kind: "stay" };
					},
	...
	autoWaitRuntime = registerCodexAutoWait(pi, settingsRuntime, {
		checkCurrentUsage: async (ctx, signal) => {
			const model = ctx.model;
			if (model?.provider !== "openai-codex") return undefined;
			const outcome = await queryCurrentState(ctx, model, true, signal);
			if (
				outcome.state.status !== "ready" ||
				outcome.state.report.providerId !== "openai-codex"
			) {
				return undefined;
			}
			return { model, report: outcome.state.report };
		},
		publishObservedUsage: (ctx, usage) => {
			const rawValue =
				formatUsageStatusline(usage.report, usage.model) ??
				"codex usage unavailable";
			const fastValue = fastRuntime.decorateStatus(usage.model, rawValue);
			safeSetStatus(
				ctx,
				autoWaitRuntime.decorateStatus(usage.model, fastValue),
			);
		},
	});
}
